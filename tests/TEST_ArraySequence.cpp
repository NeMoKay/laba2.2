...
TEST_F(ArraySequence_Fixture, array_constructor){
    ArraySequence<int> arr(data_int);
...
TEST_F(ArraySequence_Fixture, concat){
    int extra[2] ={1, 2};
    ArraySequence<int> extraSeq(extra);
...
TEST_F(ArraySequence_Fixture, map){
    ...
    EXPECT_EQ(mapped->GetLength(), 5) << "Функция выволнения map (x * 2)\n" << "Ожидаемая длинна 5. По факту : " << mapped->GetLength();
    EXPECT_EQ(mapped->Get(0), 84) <<  "Функция выволнения map (x * 2)\n" << "Ожидаемый элемент 84. По факту : " << mapped->Get(0);
    EXPECT_EQ(mapped->Get(2), 138) << "Функция выволнения map (x * 2)\n" << "Ожидаемый элемент 138. По факту : " << mapped->Get(2);
    delete mapped;
}
...
TEST_F(ArraySequence_Fixture, where){
    ...
    EXPECT_EQ(filtered->Get(0), 69) << "Функция выволнения Where (x > 60)\n" << "Ожидаемый элемент 69. По факту : " << filtered->Get(0);
    EXPECT_EQ(filtered->Get(1), 67) <<  "Функция выволнения Where (x > 60)\n" <<"Ожидаемый элемент 67. По факту : " << filtered->Get(1);
    EXPECT_EQ(filtered->Get(2), 228) <<  "Функция выволнения Where (x > 60)\n" <<"Ожидаемый элемент 228. По факту : " << filtered->Get(2);
    delete filtered;
}

TEST_F(ArraySequence_Fixture, reduce){
    int total = Reduce(seq_int, sum_func, 0);
    EXPECT_EQ(total, 42 + 52 + 69 + 67 + 228) <<  "Функция выволнения reduce (sum(xi))\n" <<"Ожидаемый элемент " << 42 + 52 + 69 + 67 + 228 << ". По факту : " << total;
}