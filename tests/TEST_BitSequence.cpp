...
TEST_F(BitSequence_Fixture, array_constructor){
    BitSequence<char> array(data_bits);
...
TEST_F(BitSequence_Fixture, concat){
    Bit<char> extra[] = {Bit<char>(1), Bit<char>(0)};
    BitSequence<char> extraSeq(extra);
...
TEST_F(BitSequence_Fixture, AND){
    ...
    BitSequence<char> other(other_bits);
    
    BitSequence<char> result = (*Sequence) & other;
    
    EXPECT_EQ(result.GetLength(), 8) << "Функция выполнения AND (&)\n" << "Ожидаемая длина 8. По факту : " << result.GetLength();
    EXPECT_EQ(bool(result.Get(0)), true)   << "Функция выполнения AND (&)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(0));
    EXPECT_EQ(bool(result.Get(1)), false)  << "Функция выполнения AND (&)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(1));
    EXPECT_EQ(bool(result.Get(2)), false)  << "Функция выполнения AND (&)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(2));
    EXPECT_EQ(bool(result.Get(7)), false)  << "Функция выполнения AND (&)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(7));
}

TEST_F(BitSequence_Fixture, AND_diff_length){
    Bit<char> short_bits[] = {Bit<char>(1), Bit<char>(0), Bit<char>(1), Bit<char>(0)};
    BitSequence<char> short_seq(short_bits);
...
TEST_F(BitSequence_Fixture, OR){
    ...
    EXPECT_EQ(result.GetLength(), 8) << "Функция выполнения OR (|)\n" << "Ожидаемая длина 8. По факту : " << result.GetLength();
    EXPECT_EQ(bool(result.Get(0)), true)   << "Функция выполнения OR (|)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(0));
    EXPECT_EQ(bool(result.Get(1)), false)  << "Функция выполнения OR (|)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(1));
    EXPECT_EQ(bool(result.Get(2)), true)   << "Функция выполнения OR (|)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(2));
    EXPECT_EQ(bool(result.Get(3)), true)   << "Функция выполнения OR (|)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(3));
}
...
TEST_F(BitSequence_Fixture, XOR){
    ...
    EXPECT_EQ(result.GetLength(), 8) << "Функция выполнения XOR (^)\n" << "Ожидаемая длина 8. По факту : " << result.GetLength();
    EXPECT_EQ(bool(result.Get(0)), false)  << "Функция выполнения XOR (^)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(0));
    EXPECT_EQ(bool(result.Get(1)), true)   << "Функция выполнения XOR (^)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(1));
    EXPECT_EQ(bool(result.Get(2)), true)   << "Функция выполнения XOR (^)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(2));
    EXPECT_EQ(bool(result.Get(4)), true)   << "Функция выполнения XOR (^)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(4));
}

TEST_F(BitSequence_Fixture, NOT){
    BitSequence<char> result = ~(*Sequence);
    
    EXPECT_EQ(result.GetLength(), 8) << "Функция выполнения NOT (~)\n" << "Ожидаемая длина 8. По факту : " << result.GetLength();
    EXPECT_EQ(bool(result.Get(0)), false)  << "Функция выполнения NOT (~)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(0));
    EXPECT_EQ(bool(result.Get(1)), true)   << "Функция выполнения NOT (~)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(1));
    EXPECT_EQ(bool(result.Get(2)), false)  << "Функция выполнения NOT (~)\n" << "Ожидаемый элемент 0. По факту : " << bool(result.Get(2));
    EXPECT_EQ(bool(result.Get(7)), true)   << "Функция выполнения NOT (~)\n" << "Ожидаемый элемент 1. По факту : " << bool(result.Get(7));
}

//Деструктор
TEST_F(BitSequence_Fixture, destructor){
    BitSequence<char>* bs = new BitSequence<char>(data_bits);
    delete bs;
    SUCCEED();
}